    ...
    <div style={{ display: 'flex', gap: 154, alignItems: 'center', justifyContent: 'center', padding: '21px 24px 19px', boxSizing: 'border-box', position: 'relative', zIndex: 20, width: '100%' }}>
      ...
      <div style={{ flex: 1, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, paddingTop: 1, paddingRight: 1 }}>
        <svg width="19" height="12" viewBox="0 0 19 12">
          <rect x="0" y="7.5" width="3.2" height="4.5" rx="0.7" fill={c}/><rect x="4.8" y="5" width="3.2" height="7" rx="0.7" fill={c}/><rect x="9.6" y="2.5" width="3.2" height="9.5" rx="0.7" fill={c}/><rect x="14.4" y="0" width="3.2" height="12" rx="0.7" fill={c}/>
        </svg>
        ...
        <svg width="27" height="13" viewBox="0 0 27 13">
          <rect x="0.5" y="0.5" width="23" height="12" rx="3.5" stroke={c} strokeOpacity="0.35" fill="none"/><rect x="2" y="2" width="20" height="9" rx="2" fill={c}/><path d="M25 4.5V8.5C25.8 8.2 26.5 7.2 26.5 6.5C26.5 5.8 25.8 4.8 25 4.5Z" fill={c} fillOpacity="0.4"/>
        </svg>
      </div>
    </div>
  );
...
interface IOSGlassPillProps { children: React.ReactNode; dark?: boolean; style?: React.CSSProperties; }
...
interface IOSNavBarProps { title?: string; dark?: boolean; trailingIcon?: boolean; }
...
interface IOSListRowProps { title: string; detail?: string; icon?: string; chevron?: boolean; isLast?: boolean; dark?: boolean; }
...
interface IOSListProps { header?: string; children: React.ReactNode; dark?: boolean; }
...
interface IOSDeviceProps { children: React.ReactNode; width?: number; height?: number; dark?: boolean; title?: string; keyboard?: boolean; }
...
interface IOSKeyboardProps { dark?: boolean; }
...
export function IOSKeyboard({ dark = false }: IOSKeyboardProps) {
  ...
  const key = (content: React.ReactNode, opts: KeyOpts) => (
    <div key={opts.k} style={{ height: 42, borderRadius: 8.5, flex: opts.flex ? 1 : undefined, width: opts.w, minWidth: 0, background: opts.ret ? '#08f' : keyBg, boxShadow: '0 1px 0 rgba(0,0,0,0.075)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: '-apple-system, "SF Compact", system-ui', fontSize: opts.fs ?? 25, fontWeight: 458, color: opts.ret ? '#fff' : glyph }}>{content}</div>
  );
  ...
  return (
    <div style={{ position: 'relative', zIndex: 15, borderRadius: 27, overflow: 'hidden', padding: '11px 0 2px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, borderRadius: 27, boxShadow: 'inset 1.5px 1.5px 1px rgba(255,255,255,0.7)', border: '0.5px solid rgba(0,0,0,0.06)', pointerEvents: 'none' }}/>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 13, padding: '0 6.5px', width: '100%', boxSizing: 'border-box', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', gap: 14.25, alignItems: 'center' }}>
          {key(svgIcons.shift, { w: 45, k: 'shift' })}
          <div style={{ display: 'flex', gap: 6.5, flex: 1 }}>
            {['z','x','c','v','b','n','m'].map(l => key(l, { flex: true, k: l }))}
          </div>
          {key(svgIcons.del, { w: 45, k: 'del' })}
        </div>
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          {key('ABC', { w: 92.25, fs: 18, k: 'abc' })}
          {key('', { flex: true, k: 'space' })}
          {key(svgIcons.ret, { w: 92.25, ret: true, k: 'ret' })}
        </div>
      </div>
